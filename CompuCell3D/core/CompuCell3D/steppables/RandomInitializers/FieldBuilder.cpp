...
FieldBuilder::FieldBuilder ( SimulatorPtr _simulator )
{
...
//		cout << xlist[i] << "," << ylist[i] << "," << zlist[i] << endl;
//	}
}
...
{
//	cout << "add cell at " << pos.x << "," << pos.y << "," << pos.z << endl;
    CellG *cell;
    map<double,int>::iterator it;
        ...
        for ( dy = 0; dy < ( size.y ); dy++ )
            for ( dz = 0; dz < ( size.z ); dz++ )
            {
                if ( ! ( ( dx==0 ) && ( dy==0 ) && ( dz==0 ) ) )
                {
                    ...
                    p.y = pos.y+dy;
                    p.z = pos.z+dz;
//					cout << "add cell at " << p.x << "," << p.y << "," << p.z << endl;
...
void FieldBuilder::setTypeVec ( int ncells, vector<string> typeNames,vector<string> biasVec )
{
    ...
    for ( i=0; i<sz; i++ )
    {
        ...
        if ( val < 1 )
        {
            typeVec.insert ( typeVec.begin(),floor ( ncells*val ),tid );
        }
        else if ( ( val == 1 ) && ( valsum == 1 ) )
        {
            typeVec.insert ( typeVec.begin(),floor ( ncells*val ),tid );
        }
        else
        {
            typeVec.insert ( typeVec.begin(),floor ( val ),tid );
        }
    }
    i = 0;
    while ( typeVec.size() < ncells )
    {
//		cout << "add cells of type " << (int)automaton->getTypeId(typeNames[i]) << endl;
...
void FieldBuilder::setTypeVec ( int ncells,vector<string> typeNames )
{
    ...
    {
        tid = ( int ) automaton->getTypeId ( typeNames[i] );
//		cout << "add cells of type " << tid << endl;
