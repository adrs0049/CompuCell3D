...
template <typename GPU_Solver>
FlexibleDiffusionSolverFE_GPU<GPU_Solver>::FlexibleDiffusionSolverFE_GPU()
	: DiffusableVectorCommon<float, Array3DCUDA>(),deltaX(1.0),deltaT(1.0), gpuDevice(new GPU_Solver()), gpuDeviceIndex(-1)
{
	serializerPtr=0;
	pUtils=0;
	serializeFlag=false;
...
void FlexibleDiffusionSolverFE_GPU<GPU_Solver>::init(Simulator *_simulator, CC3DXMLElement *_xmlData) {
	...
	boundaryMonitorPlugin=(BoundaryMonitorPlugin*)Simulator::pluginManager.get("BoundaryMonitor",&pluginAlreadyRegisteredFlag);
	if(!pluginAlreadyRegisteredFlag){
...
//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
template <typename GPU_Solver>
float FlexibleDiffusionSolverFE_GPU<GPU_Solver>::couplingTerm(Point3D & _pt,std::vector<CouplingData> & _couplDataVec,float _currentConcentration){
			...
			if (bcSpec.planePositions[0]==BoundaryConditionSpecifier::CONSTANT_VALUE){
				...
				for(int y=0 ; y< workFieldDim.y-1; ++y)
					for(int z=0 ; z<workFieldDim.z-1 ; ++z){
						...
						h_celltype_field->setDirect(x,y,z,h_celltype_field->getDirect(x+1,y,z)); //for const value and const derivative cell type array is initialized in the same way as we do it for no flux oundary conditions
						h_boundary_field->setDirect(x,y,z,h_boundary_field->getDirect(x+1,y,z));
					}
...
//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
template <typename GPU_Solver>
void FlexibleDiffusionSolverFE_GPU<GPU_Solver>::scrarch2Concentration( ConcentrationField_t *scratchField, ConcentrationField_t *concentrationField){
	//scratchField->switchContainersQuick(*(concentrationField));
}
//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
template <typename GPU_Solver>
void FlexibleDiffusionSolverFE_GPU<GPU_Solver>::outputField( std::ostream & _out, ConcentrationField_t *_concentrationField){
	Point3D pt;
	float tempValue;
...
//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
template <typename GPU_Solver>
void FlexibleDiffusionSolverFE_GPU<GPU_Solver>::readConcentrationField(std::string fileName,ConcentrationField_t *concentrationField){
