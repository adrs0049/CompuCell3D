			...
			for (int z = 0; z < fieldDim.z; z++){
				...
				if(cell){ //do not secrete in medium				
					...
					if(cell->type<oxygenSecretionData.size() && oxygenSecretionData[cell->type].dataInitialized){
						...
						//cerr<<"pt="<<pt<<" newConcentraion="<<newConcentration<<" current concentration="<<currentConcentration<<endl;
						concentrationFieldPtr->set(pt,-newConcentration);		//minus sign is added because of the convention implied by pde solving fcn - deltaT scaling is done in diffuseSingleField function		 
	...
	for (int x = 0; x < fieldDim.x; x++)
		for (int y = 0; y < fieldDim.y; y++)  
			...
			if(secreteInMedium && ! cell){
				concentrationFieldPtr->set(pt,-secrConstMedium);//minus sign is added because of the convention implied by pde solving fcn - deltaT scaling is done in diffuseSingleField function				
				...
				if(uptakeInMediumFlag && ! cell){						
					if (relativeUptakeRateInMedium){ //doing relative uptake
						if(currentConcentration*relativeUptakeRateInMedium>maxUptakeInMedium){
							...
							concentrationFieldPtr->set(pt,currentConcentration*relativeUptakeRateInMedium); //positive value here means uptake
							pixelUntouched=false;
						}
					}else{ //doing  MichaelisMenten coef-based type of uptake
						
						concentrationFieldPtr->set(pt,maxUptakeInMedium*(currentConcentration/(currentConcentration+mmCoefInMedium)));//positive value here means uptake
						pixelUntouched=false;					
				...
				if(cell){
					...
					if(mitrUptake!=end_mitrUptake){				
						if (mitrUptake->second.relativeUptakeRate){ //doing relative uptake
							if(currentConcentration*mitrUptake->second.relativeUptakeRate > mitrUptake->second.maxUptake){//positive value here means uptake
								concentrationFieldPtr->set(pt,mitrUptake->second.maxUptake);
								pixelUntouched=false;								
							}else{
								concentrationFieldPtr->set(pt,currentConcentration*mitrUptake->second.relativeUptakeRate);//positive value here means uptake
								pixelUntouched=false;
							...
							concentrationFieldPtr->set(pt,mitrUptake->second.maxUptake*(currentConcentration/(currentConcentration+mitrUptake->second.mmCoef)));//positive value here means uptake
							pixelUntouched=false;					
						}
					}
				}
...
void SteadyStateDiffusionSolver::diffuseSingleField(unsigned int idx){
	...
	if (idx<bcSpecVec.size()){
		...
		if (bcSpec.planePositions[4]==BoundaryConditionSpecifier::PERIODIC || bcSpec.planePositions[5]==BoundaryConditionSpecifier::PERIODIC){
			...
			//the solution is specified at X = A and X = B
			Point3D pt;
			if (bcSpec.planePositions[4]==BoundaryConditionSpecifier::CONSTANT_VALUE && bcSpec.planePositions[5]==BoundaryConditionSpecifier::CONSTANT_VALUE){
			...
			}else if (bcSpec.planePositions[4]==BoundaryConditionSpecifier::CONSTANT_VALUE && bcSpec.planePositions[5]==BoundaryConditionSpecifier::CONSTANT_DERIVATIVE){
			...
			}else if (bcSpec.planePositions[4]==BoundaryConditionSpecifier::CONSTANT_DERIVATIVE && bcSpec.planePositions[5]==BoundaryConditionSpecifier::CONSTANT_DERIVATIVE){
			...
			}else if (bcSpec.planePositions[4]==BoundaryConditionSpecifier::CONSTANT_DERIVATIVE && bcSpec.planePositions[5]==BoundaryConditionSpecifier::CONSTANT_VALUE){
...
//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
void SteadyStateDiffusionSolver::readConcentrationField(std::string fileName,ConcentrationField_t *concentrationField){
