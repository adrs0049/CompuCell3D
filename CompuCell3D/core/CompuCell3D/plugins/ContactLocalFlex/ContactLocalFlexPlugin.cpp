...
void ContactLocalFlexPlugin::init ( SimulatorPtr simulator, CC3DXMLElement *_xmlData )
{
    xmlData=_xmlData;
    sim=simulator;
    ...
    potts->getCellFactoryGroupPtr()->registerClass ( std::make_shared<DataAccessor_t> ( contactDataContainerAccessor ) );


    bool pluginAlreadyRegisteredFlag;
    auto plugin=Simulator::pluginManager.get ( "NeighborTracker",&pluginAlreadyRegisteredFlag ); //this will load SurfaceTracker plugin if it is not already loaded
...
void ContactLocalFlexPlugin::extraInit ( SimulatorPtr simulator )
{
    update ( xmlData, true );
}
...
void ContactLocalFlexPlugin::initializeContactLocalFlexData()
{
    ...
    for ( cInvItr=cellInventoryPtr->cellInventoryBegin() ; cInvItr !=cellInventoryPtr->cellInventoryEnd() ; ++cInvItr )
    {
        ...
        //cell=*cInvItr;
        auto dataContainer = contactDataContainerAccessor.get ( cell->extraAttribPtr );
...
void ContactLocalFlexPlugin::updateContactEnergyData ( CellG *_cell )
{
    ...
    auto neighborTrackerPlugin=std::static_pointer_cast<NeighborTrackerPlugin> ( Simulator::pluginManager.get ( "NeighborTracker" ) );
    auto neighborTrackerAccessorPtr = neighborTrackerPlugin->getNeighborTrackerAccessorPtr();
    ...
    //unsigned int size1=clfdSet.size();
    //unsigned int size2=nsdSet.size();
	//
