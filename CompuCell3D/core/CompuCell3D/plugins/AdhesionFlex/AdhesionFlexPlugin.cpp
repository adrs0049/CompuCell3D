...
AdhesionFlexPlugin::AdhesionFlexPlugin()
    : pUtils ( nullptr ), lockPtr ( nullptr ), xmlData ( nullptr ),
      potts ( nullptr ), sim ( nullptr ), boundaryStrategy ( nullptr ),
      adhesionFlexDataAccessorPtr ( nullptr ), automaton ( nullptr ),
      adhesionFlexEnergyPtr ( &AdhesionFlexPlugin::adhesionFlexEnergyCustom ),
      weightDistance ( false ), adhesionDensityInitialized ( false ), numberOfAdhesionMolecules ( 0 )
{}
...
void AdhesionFlexPlugin::handleEvent ( CC3DEvent & _event )
{
    if ( _event.id==CHANGE_NUMBER_OF_WORK_NODES )
    {
        ...
        for ( unsigned int i  = 0 ; i< maxNumberOfWorkNodes ; ++i )
        {
...
void AdhesionFlexPlugin::update ( CC3DXMLElement *_xmlData, bool _fullInitFlag )
{
    ...
    for ( unsigned int i = 0 ; i<adhesionMoleculeNameXMLVec.size(); ++i )
    {
    ...
    for ( unsigned int i = 0 ; i<adhesionMoleculeDensityXMLVec.size(); ++i )
    {
    ...
    for ( unsigned int i = 0 ; i<bindingParameterXMLVec.size(); ++i )
    {
    ...
    unsigned int sizeBindingArray=moleculeNameIndexMap.size();
    ...
    for ( unsigned int i = 0 ; i < sizeBindingArray ; ++i )
        for ( unsigned int j = 0 ; j < sizeBindingArray ; ++j )
        {
