...
//   chemicalEnergy->setSimulatorPtr(simulator);
     ...
     auto plugin=Simulator::pluginManager.get ( "NeighborTracker",&pluginAlreadyRegisteredFlagNeighbor );
     if ( !pluginAlreadyRegisteredFlagNeighbor )
          plugin->init ( sim );


     bool pluginAlreadyRegisteredFlag;
     auto simpleClockPlugin=std::static_pointer_cast<SimpleClockPlugin> ( Simulator::pluginManager.get ( "SimpleClock",&pluginAlreadyRegisteredFlag ) ); //this will load SurfaceTracker plugin if it is not already loaded
...
void ChemotaxisDictyPlugin::initializeField()
{
     if ( !gotChemicalField ) { ///this is only temporary solution will have to come up with something better
          ...
          auto steppable=classRegistry->getStepper ( chemicalFieldSource );
          //if(chemicalFieldSource=="DiffusionSolverBiofilmFE"){
          field= ( ( DiffusableVector<float> * ) steppable.get() )->getConcentrationField ( chemicalFieldName );
