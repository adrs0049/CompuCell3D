...
void PolarizationVectorPlugin::init ( SimulatorPtr simulator, CC3DXMLElement *_xmlData )
{
    Potts3D *potts = simulator->getPotts();

    polarizationVectorAccessor = std::make_shared<BasicClassAccessor<PolarizationVector> >();
    potts->getCellFactoryGroupPtr()->registerClass ( polarizationVectorAccessor );
}

void PolarizationVectorPlugin::extraInit ( SimulatorPtr simulator )
{}