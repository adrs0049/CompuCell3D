    ...
    if (oldCell){
        ...
        if(oldCell->volume>1){
            ...
            oldCOMAfter.fZ=centroidOldAfter.Z()/(float)(oldCell->volume-1);
                ...
                //          return 0.0;//if oldCell is to disappear t
//             oldPolVecAfter.fX=0.0;
//             oldPolVecAfter.fY=0.0;
//             oldPolVecAfter.fZ=0.0;
...
//                 newPolVecAfter=Vector3(0.0,0.0,0.0);
    ...
    if (oldCell && newCell && oldCell->clusterId==newCell->clusterId){
        //we will use before and after polarization vectors from oldCell - this will automatically handle the situation of oldCell disappearing
        double eBefore=oldPolData.lambda*(oldPolData.polarizationVec-oldPolVecBefore)*(oldPolData.polarizationVec-oldPolVecBefore);    
        double eAfter=oldPolData.lambda*(oldPolData.polarizationVec-oldPolVecAfter)*(oldPolData.polarizationVec-oldPolVecAfter);            
        energy+=(eAfter-eBefore);            
    }else{
        if (newCell){
            double eBefore=newPolData.lambda*(newPolData.polarizationVec-newPolVecBefore)*(newPolData.polarizationVec-newPolVecBefore); 
            double eAfter=newPolData.lambda*(newPolData.polarizationVec-newPolVecAfter)*(newPolData.polarizationVec-newPolVecAfter);                            
            energy+=(eAfter-eBefore);            
        }
        if (oldCell){
            double eBefore=oldPolData.lambda*(oldPolData.polarizationVec-oldPolVecBefore)*(oldPolData.polarizationVec-oldPolVecBefore);    
