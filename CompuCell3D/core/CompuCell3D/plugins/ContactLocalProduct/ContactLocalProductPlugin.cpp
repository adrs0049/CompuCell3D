...
void ContactLocalProductPlugin::update ( CC3DXMLElement *_xmlData, bool _fullInitFlag )
{
    ...
    if ( _xmlData->findElement ( "CustomFunction" ) )
    {
        ...
        for ( unsigned int i = 0 ; i<variableVec.size(); ++i )
        {
            ...
            if ( variableCount==0 )
            {
                ...
                for ( unsigned int idx  = 0 ; idx< maxNumberOfWorkNodes ; ++idx )
                {
...
std::string ContactLocalProductPlugin::toString()
{
    return "ContactLocalProduct";
}