...
void Potts3D::registerAutomaton ( Automaton* autom )
{
    automaton = autom;
}

Automaton* Potts3D::getAutomaton()
{
    return automaton;
}
...
EnergyFunction * Potts3D::getConnectivityConstraint()
{
...
void Potts3D::registerAcceptanceFunction ( std::unique_ptr<AcceptanceFunction> function )
{
...
unsigned int Potts3D::metropolisFast ( const unsigned int steps, const double temp )
{
    ...
    for ( unsigned int i  = 0 ; i < subgridSectionOrderVec.size() ; ++i )
    {
    ...
    #pragma omp parallel
    {
        unsigned int numberOfAttemptsLocal;
        ...
        for ( unsigned int s = 0 ; s<subgridSectionOrderVec.size() ; ++s )
        {
            ...
            numberOfAttemptsLocal= ( unsigned int ) ( sectionDims.second.x-sectionDims.first.x ) * ( sectionDims.second.y-sectionDims.first.y ) * ( sectionDims.second.z-sectionDims.first.z ) *sim->getFlip2DimRatio();
